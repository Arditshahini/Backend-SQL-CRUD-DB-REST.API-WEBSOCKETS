...
const db = pgp('postgres://dittanvändarnamn:dittlösenord@localhost:5432/dvdrental')
...
async function updateCustomer(customerId, firstName, lastName, email) {
  ...
  await db.none(`UPDATE customer SET first_name = '${firstName}', last_name = '${lastName}', email = '${email}' WHERE customer_id = ${customerId}`);
}
